...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let hardReset = false;       // full reset request
  let prevReady = false;
  ...
  const P1 = { x: 550, y: 280 };
  const P2 = { x: 400, y: 240 };
  const P3 = { x: 400, y: 280 };
  const P4 = { x: 250, y: 280 };
  ...
  function moveToolTo(pt, steps = 60, ms = 30) {
    return new Promise(async (res) => {
      ...
      for (let i = 1; i <= steps; i++) {
        if (stopFlag) break; // يسمح بالـPause وسط الحركة
  ...
  function animateBarResumable(idBar, idTxt, totalMs, remainMsRef){
    if (remainMsRef.v == null) remainMsRef.v = totalMs;
  ...
  function ensureCountersUI(){
    if (!$('cyclesDone') || !$('currentCycle') || !$('stepLabel')){
      const controlsCard = document.querySelector('.card.card--controls') || document.querySelectorAll('.card')[1] || document.body;
